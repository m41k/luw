...
#	distro=( `cat /opt/luw/repo/index-user | awk -F";" {'print $1"_"$2"_"$3'}`)
	distro=( `cat /opt/luw/repo/cardapio.luw | awk -F";" {'print $1"_"$2"_"$3'}`)
...
#	echo "mkdir -p $HOME/.cache/lxc/download/$distro/$release/$arquit/default/"
#	echo "cp /tmp/repo/images/$distro/$release/$arquit/default/* $HOME/.cache/lxc/download/$distro/$release/$arquit/default/"
#       echo "tar -Jxf $HOME/.cache/lxc/download/$distro/$release/$arquit/default/meta.tar.xz -C $HOME/.cache/lxc/download/$distro/$release/$arquit/default/"
...
#
#	echo "$ssh lxc-create -t download -n $contname -- -d $distro -r $release -a $arquit 2> /dev/null"
