...
echo "#!/bin/bash" > ~/public_html/cgi-bin/luw-box/$container.sh
echo shellinaboxd --no-beep --cgi -t -s '"'/:$(id -u):$(id -g):HOME:/bin/bash /home/$REMOTE_USER/public_html/cgi-bin/luw-box/$container.box'"' >> ~/public_html/cgi-bin/luw-box/$container.sh
echo lxc-console -q -n $container > ~/public_html/cgi-bin/luw-box/$container.box
chmod 705 ~/public_html/cgi-bin/luw-box/$container.sh
chmod +x ~/public_html/cgi-bin/luw-box/$container.sh
goshell='<meta http-equiv="refresh" content="0;url=http://'$SERVER_NAME'/~'$REMOTE_USER'/cgi-bin/luw-box/'$container.sh'">'
echo $goshell
